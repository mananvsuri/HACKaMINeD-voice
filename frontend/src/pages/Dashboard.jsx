    ...
    if (active && payload && payload.length) {
        ...
        return (
            <div className="glass-panel" style={{ padding: '1rem', minWidth: '200px' }}>
                <p style={{ fontWeight: 600, color: 'var(--text)', fontSize: '1.1rem', marginBottom: '8px', borderBottom: '1px solid var(--panel-border)', paddingBottom: '4px', display: 'flex', justifyContent: 'space-between' }}>
                    {data.name}
    ...
    return (
        <div className="dashboard-container" style={{ maxWidth: '1400px', margin: '0 auto' }}>
            <header style={{ marginBottom: '2.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'rgba(16, 185, 129, 0.1)', color: 'var(--success)', padding: '0.4rem 0.8rem', borderRadius: '6px', fontSize: '0.85rem', fontWeight: 600, border: '1px solid rgba(16, 185, 129, 0.2)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.5rem', marginBottom: '2.5rem' }}>
                <div className="glass-panel" style={{ position: 'relative', overflow: 'hidden' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--text-muted)', marginBottom: '1rem' }}>
                        <span style={{ fontWeight: 500, fontSize: '0.95rem', color: 'var(--text-muted)' }}>Gross Revenue</span>
                ...
                <div className="glass-panel" style={{ position: 'relative', overflow: 'hidden' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--text-muted)', marginBottom: '1rem' }}>
                        <span style={{ fontWeight: 500, fontSize: '0.95rem', color: 'var(--text-muted)' }}>Items Sold</span>
                ...
                <div className="glass-panel" style={{ position: 'relative', overflow: 'hidden' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--text-muted)', marginBottom: '1rem' }}>
                        <span style={{ fontWeight: 500, fontSize: '0.95rem', color: 'var(--text-muted)' }}>Avg Profit Margin</span>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.8fr) minmax(0, 1fr)', gap: '2rem', marginBottom: '2.5rem' }}>
                ...
                <div className="glass-panel" style={{ display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ flex: 1, minHeight: '350px' }}>
                        <ResponsiveContainer width="100%" height="100%">
                            <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                                <CartesianGrid strokeDasharray="4 4" stroke="var(--panel-border)" vertical={false} />
                                <XAxis type="number" dataKey="sales_velocity" name="Velocity" unit=" /day" stroke="var(--text-muted)" tick={{ fill: 'var(--text-muted)', fontSize: 11 }} tickLine={false} axisLine={{ stroke: 'var(--panel-border)' }} />
                                <YAxis type="number" dataKey="margin_percentage" name="Margin" unit="%" stroke="var(--text-muted)" tick={{ fill: 'var(--text-muted)', fontSize: 11 }} tickLine={false} axisLine={{ stroke: 'var(--panel-border)' }} />
                                <Tooltip content={<CustomTooltip />} cursor={{ fill: 'var(--surface-light)', opacity: 0.4 }} />
                                <Scatter name="Menu Items" data={insights} fill="var(--primary)">
                                    {insights.map((entry, index) => {
                                        ...
                                        const radius = Math.max(4, Math.min(12, (entry.total_revenue / 1000) * 5));
                                        return <Cell key={`cell-${index}`} fill={color} r={radius} fillOpacity={0.8} />;
                                    })}
                                </Scatter>
                            </ScatterChart>
                        </ResponsiveContainer>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'center', gap: '1.5rem', marginTop: '1.5rem', fontSize: '0.9rem', color: 'var(--text-muted)', flexWrap: 'wrap' }}>
                        <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}><div style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--success)' }}></div> Star Items</span>
                        <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}><div style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--secondary)' }}></div> High Margin / Low Vol</span>
                        <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}><div style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--danger)' }}></div> Low Margin / High Vol</span>
                    </div>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', maxHeight: '600px', overflowY: 'auto' }}>
                        ...
                        <div className="glass-panel" style={{ display: 'flex', flexDirection: 'column' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                                {lowStockAlerts.map(item => (
                                    <div key={item.id} style={{ background: 'var(--surface)', border: '1px solid var(--panel-border)', borderLeft: '3px solid var(--primary)', borderRadius: '6px', padding: '1rem' }}>
                    ...
                    <div className="glass-panel" style={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        <div style={{ overflowY: 'auto', paddingRight: '0.5rem', display: 'flex', flexDirection: 'column', gap: '1rem', flex: 1 }}>
                            {toPromote.map(item => (
                                <div key={item.id} style={{ background: 'var(--surface)', border: '1px solid var(--panel-border)', borderLeft: '3px solid var(--secondary)', borderRadius: '6px', padding: '1rem', transition: 'all 0.2s', cursor: 'pointer' }} onMouseEnter={e => e.currentTarget.style.borderColor = 'var(--secondary)'} onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--panel-border)'}>
                            ...
                            {toReprice.map(item => (
                                <div key={item.id} style={{ background: 'var(--surface)', border: '1px solid var(--panel-border)', borderLeft: '3px solid var(--danger)', borderRadius: '6px', padding: '1rem', transition: 'all 0.2s', cursor: 'pointer' }} onMouseEnter={e => e.currentTarget.style.borderColor = 'var(--danger)'} onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--panel-border)'}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                ...
                <div className="glass-panel">
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '1.5rem' }}>
                        {combos.map((combo, idx) => (
                            <div key={idx} style={{ padding: '1.5rem', background: 'var(--surface)', borderRadius: '8px', border: '1px solid var(--panel-border)', transition: 'all 0.2s' }}>
